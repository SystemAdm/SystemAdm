<template>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 w-full">
        <div class="relative mx-auto w-full p-5" v-for="event in events" :key="event.id">
            <a href="#"
                ...
                <div class="shadow rounded-lg bg-gray-300"
                    ...
                    <div class="flex justify-center relative rounded-lg overflow-hidden h-52">
                        ...
                        <span v-if="event.private"
                              class="absolute top-0 left-0 inline-flex  px-3 py-2 rounded-br-lg z-10 bg-yellow-600 text-sm font-medium text-white select-none uppercase">Crew only</span>
                        <span v-else-if="event.protected"
                              class="absolute top-0 left-0 inline-flex px-3 py-2 rounded-lg z-10 bg-blue-600 text-sm font-medium text-white select-none uppercase">Members only</span>
                        ...
                        <span v-if="event.rating === 'age' && event.rating_max != null" class="block px-3 pt-2 text-right bg-blue-600 text-sm font-medium text-white select-none uppercase">Max: {{event.rating_max}} år</span>
                        <span v-if="event.rating === 'age' && event.rating_min != null" class="block px-3 py-2 text-right rounded-bl-lg bg-blue-600 text-sm font-medium text-white select-none uppercase">Min: {{event.rating_min}} år</span>
                            <span v-if="event.rating === 'grade' && event.rating_max != null" class="block px-3 pt-2 text-right bg-blue-600 text-sm font-medium text-white select-none uppercase">Max: {{event.rating_max}}. klasse</span>
                            <span v-if="event.rating === 'grade' && event.rating_min != null" class="block px-3 py-2 text-right rounded-bl-lg bg-blue-600 text-sm font-medium text-white select-none uppercase">Min: {{event.rating_min}}. klasse</span>
                        </div>
                    </div>
                    ...
                    <div v-if="event.duration_days > 0" class="grid gap-2 p-4">
                        <p class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                            <span class="font-medium">Event begin:</span>
                            <span class="xl:mt-0">{{ event.event_begin_date }}</span>
                        </p>
                        <p class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                            <span class="font-medium">Event end:</span>
                    ...
                    <div class="p-4">
                        <div v-if="event.cancelled">
                            <div class="hover:cursor-default block rounded-2xl py-2 px-3 mb-2 text-center bg-red-600">
                                Cancelled:<span class="block font-extrabold">{{ event.cancelled_text }}</span>
                            </div>
                        </div>
                        <div v-else-if="event.planning">
                            <div
                                class="hover:cursor-default block rounded-2xl py-2 px-3 mb-2 text-center bg-yellow-600">
                                Status:<span class="block font-extrabold">In planning mode</span>
                            </div>
                        </div>
                        <div v-else>
                            <div v-if="event.seats_available !== false && event.seats_available !== true">
                                <div :class="{'bg-red-500 ':event.seats_available <= 0}"
                                    ...
                                    {{ event.seats_available }} of {{ event.seats }} available
                                </div>
                            </div>
    ...
    <EventSignup :modalOpen="modalOpen" @update:modalOpen="modalOpen = $event" :selected="selected"></EventSignup>
</template>
<script>
...
export default {
    components: {EventSignup},
    data() {
        return {
            pegi: {3: 'age-3.jpg', 7: 'age-7.jpg', 12: 'age-12.jpg', 16: 'age-16.jpg', 18: 'age-18.jpg'},
