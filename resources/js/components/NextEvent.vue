			...
			Featured
                    ...
                    <div class="grid grid-cols-2 grid-rows-2 gap-4 mt-8">
                        <p class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                            <span class="font-bold">Event begin</span>
                            <span class="mt-2 xl:mt-0">{{ event.event_begin_date }}</span>
                        </p>
                        <p class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                            <span class="font-bold">Event end</span>
                            <span class="mt-2 xl:mt-0">{{ event.event_end_date }}</span>
                        </p>
                        <p v-if="!event.signup_started" class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                            <span class="font-bold">Registration begin</span>
                            <span class="mt-2 xl:mt-0">{{ event.signup_begin_date }}</span>
                        </p>
                        <p v-if="!event.signup_ended" class="inline-flex flex-col xl:flex-row xl:items-center text-gray-800">
                            <span class="font-bold">Registration end</span>
                            <span class="mt-2 xl:mt-0">{{ event.signup_end_date }}</span>
                        </p>
                    </div>
    ...
    methods: {
        fetch() {
            axios.get('/api/events/short').then(response => {
                ...
                console.log('loaded: ' + this.events.length);
            }).catch(error => {
